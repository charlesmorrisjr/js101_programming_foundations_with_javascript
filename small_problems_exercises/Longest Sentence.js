...
function longestSentence(text) {
  ...
  let sentenceStart = 0;
  let wordCount = 0;
  let sentenceWithMostWords = 0;

  for (let idx = 0; idx < text.length; idx++) {
    if (/[.?!]/.test(text[idx])) {
      sentences.push(text.slice(sentenceStart, idx + 1));
      sentenceStart = idx + 2;

      let sentenceWordCount = sentences[sentences.length - 1].split(' ').length;
      if (sentenceWordCount > wordCount) {
        sentenceWithMostWords = sentences.length - 1;
        wordCount = sentenceWordCount;
      }
    }
  }

  console.log(sentences[sentenceWithMostWords]);
  console.log('');
  console.log(`The longest sentence has ${wordCount} words.`);
}

function hasMoreWords(sentence) {
